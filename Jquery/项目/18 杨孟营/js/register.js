$(document).ready(function() {
			$("#registerBtn").click(function() {
					...
					if(email.validity.valueMissing == true) {
						...
						email.setCustomValidity("邮箱格式不正确，例如web@bydqn.cn");
					} else {
					...
					if(uName.validity.valueMissing == true) {
						uName.setCustomValidity("昵称不能为空");
							...
							uName.setCustomValidity("昵称必须是4~20位英文和数字");
						} else {
							uName.setCustomValidity("");
						}
						var pwd = document.getElementById("pwd");
						if(pwd.validity.validity.valueMissing == true) {
						...
						if(repwd.val() == pwd.val()) {
							...
							repwd.setCustomValidity("两次密码输入的不一样");
						}
				});
