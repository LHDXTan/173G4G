$(document).ready(function(){
	$("#registerBtn").click(function(){
		...
		if(email.validity.valueMissing==true){
			...
			email.setCustomValidity("邮箱格式不对,例如web@sohu.com");
		}
		else{
		...
		if(pd.validity.valueMissing==true){
			pd.setCustomValidity("密码不能为空");
		}
		else if(pd.validity.patternMismatch==true){
			pd.setCustomValidity("密码必须是6~20位的英文和数字");
		}
		else{
			pd.setCustomValidity("");
		}
		//验证两次密码是否一致
		var pds=document.getElementById("repwd");
		if(pds.val()==pd.val()){
			pds.setCustomValidity("");
		}
		else{
			pds.setCustomValidity("两次密码不一致")
		}
