...
$("#registerBtn").click(function(){
	...
	if(email.validity.valueMissing == true) {
		...
		email.setCustomValidity("邮箱格式不正确，例如说web@bdqm.cn");
	} else {
	...
	if(uName.validity.valueMissing == true) {
		...
		uName.setCustomValidity("昵称必须是4~20 位的英文和数字");
	} else {
	...
	} else if(pwd.validity.valueMissing == true) {
	...
	if(repwd.val() == pwd.val()) {
		...
		repwd.setCustomValidity("两次输入的密码吗不一致");
	}
});
//提交按钮样式变化
