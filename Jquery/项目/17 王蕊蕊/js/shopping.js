$(document).ready(function() {
	...
	function totalPrice() {
		...
		$("#myTableProduct").find("tr").each(function(i, ele) {
			...
			var price=parseInt($(ele).find(".shopping_commend_list_4").find("label").text().replace(".","")*num);
			prices+=price;
			var prePrice=parseInt($(ele).find(".shopping_commend_list_3").find("label").text().replace(".","")*num);
			prePrices+=prePrice;
		});
